...
int main(int argc, char *argv[]) {
  ...
  for (size_t c = 0; c < buf.size(); ++c) {
    ...
    buf[c] -= 128;
    blkproc(buf_f[c], block_dct2);
  }

  for (size_t c = 0; c < buf.size(); ++c) {
    blkproc(buf_f[c], block_idct2);
    buf[c] += 128;
...
void block_dct2(cv::Mat &in) {
  if (in.rows != DCTSIZE || in.cols != DCTSIZE || in.channels() != 1) {
    printf("input for block_dct2() shall be 8*8 and monochrome.\n");
    exit(EXIT_FAILURE);
  }
  cv::dct(in, in);
}
...
void block_idct2(cv::Mat &in) {
  if (in.rows != DCTSIZE || in.cols != DCTSIZE || in.channels() != 1) {
    printf("input for block_idct2() shall be 8*8 and monochrome.\n");
    exit(EXIT_FAILURE);
  }
  cv::idct(in, in);
}